...
def store_match(regex, dt_hourly_str, after_timestamp, layer_map, all_regex_matches):
...
def main():
  ...
  for i in range(0, len(lines), skip):
    ...
    for regex in sorted(regexes, key=lambda s: -len(s)):
      ...
      if match:
        store_match(regex, dt_hourly_str, after_timestamp, layer_map, all_regex_matches)
        break
    else:
      # store_match('unmatched', dt_hourly_str, after_timestamp, layer_map, all_regex_matches)
